...
static int rad_panel_probe(struct mipi_dsi_device *dsi)
{
	...
	dsi->mode_flags =  MIPI_DSI_MODE_VIDEO_HSE | MIPI_DSI_MODE_VIDEO |
					   MIPI_DSI_CLOCK_NON_CONTINUOUS | MIPI_DSI_MODE_VIDEO_SYNC_PULSE;

	ret = of_property_read_u32(np, "video-mode", &panel->video-mode);
	if (!ret) {
		switch (video_mode) {
		case 0:
			/* burst mode */
	...
	ret = of_property_read_u32(np, "dsi-lanes", &panel->lanes);
	if (ret < 0) {
		dev_err(dev, "Failed to get dsi-lanes property (%d)\n", ret);
		return ret;
	}

	ret = videomode_from_timing(&rad_default_timing, &panel->vm);
	if (ret < 0)
		dev_err(dev, "Failed to get dsi-vm property (%d)\n", ret);
